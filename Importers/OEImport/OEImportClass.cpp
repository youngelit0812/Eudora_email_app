	...
	while (findEntryByToken(fFolders))	// Found a token
	{
		...
		if (Buffer[0] != 0)
		{
			...
			if (m_iOEVersion == OE_5_VERSION)
			{
				...
				else if (_stricmp(lpszFloater + strlen(lpszFloater) - 4, ".dbx") != 0)
				{
				...
				else
				{
					...
					if (mbpTemp)
					{
						do
						{
							if ((_stricmp(mbpTemp->lpszFilename, lpszFloater) == 0))
							{
								bValidMailbox = false;
							}
...
char * OEImportClass::MakeMimeWithFile(char *lpszFilename, void * pID, unsigned int iSize)
{
	...
	if(_stricmp(floater, "dbx") == 0)
	{
		m_iOEVersion = OE_5_VERSION;
	}
	else
	if(_stricmp(floater, "mbx") == 0)
	{
