			...
			else if( szAttributeList[0].CompareNoCase( s_szIMG ) == 0 ) 
			{
				...
				for (u = 1; u + 2 < uMax; u++)
				{
					...
					if ( (szAttributeList[u].CompareNoCase(s_szEudoraTag) == 0) &&
						 (szAttributeList[u + 1] == "=") )
					{
						CString		csAttributeValue = szAttributeList[u + 2];

						csAttributeValue.TrimLeft('"');
						csAttributeValue.TrimRight('"');

						if (csAttributeValue.CompareNoCase("emoticon") == 0)
						{
							bIsEmoticon = true;
							break;
						}
	...
	while( ( ulTotalLength > 0 ) && ( *szText != 0 ) )
	{
		...
		if (!m_bTextOnLine && (bAddSpace || ulTotalLength > 0))
		{
			...
			if (m_bDoFormatFlowed && !bAddSpace)
			{
				...
				if ((ulTotalLength >= 5 && _strnicmp(szText, "&nbsp", 5) == 0) ||
					(ulTotalLength >= 3 && _strnicmp(szText, "&gt", 3) == 0) ||
